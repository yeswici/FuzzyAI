    ...
    def _verify_supported_models(self) -> None:
        ...
        for llm in self._llms:
            if not isinstance(llm, supported_models):
                raise ValueError(f"Genetic attack mode supported LLMs are: {supported_models}. {llm.qualified_model_name} is not supported. Attack wiki: {WIKI_LINK}")
    ...
    def _verify_supported_classifiers(self) -> None:
        ...
        for classifier in self._classifiers:
            if not isinstance(classifier, supported_classifiers):
                raise ValueError(
                    f"Genetic attack mode supported classifiers are: {supported_classifiers}. {classifier.name} is not supported. Attack wiki: {WIKI_LINK}"
                )
